...
class ServicelistImport implements ToCollection, WithHeadingRow
{
  use ItemImport;
  ...
  public function collection(Collection $rows)
  {
    ...
    $logFile = $this->getLogFile('servicelist');

    $errors = $this->validate($rows);

    $this->checkForErrors('servicelist', $errors);
  ...
  private function import(Collection $rows)
  {
    ...
    foreach ($rows as $row) {
      $servicelist = Servicelist::forceCreate([
  ...
  private function validate(Collection $rows)
  {
    ...
    foreach ($rows as $index => $row) {
      ...
      if (!$title) {
        $errors[] = $this->setError($rowNo, trans('master.servicelist.props.name'), 'required');
      } elseif (strlen($title) < 2 || strlen($title) > 500) {
        $errors[] = $this->setError($rowNo, trans('master.servicelist.props.name'), 'min_max', ['min' => 2, 'max' => 500]);
      }
    }
