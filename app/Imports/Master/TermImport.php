...
class TermImport implements ToCollection, WithHeadingRow
{
  use ItemImport;
  ...
  public function collection(Collection $rows)
  {
    ...
    $logFile = $this->getLogFile('term');

    $errors = $this->validate($rows);

    $this->checkForErrors('term', $errors);
  ...
  private function validate(Collection $rows)
  {
    ...
    foreach ($rows as $index => $row) {
      ...
      if (!$title) {
        $errors[] = $this->setError($rowNo, trans('master.term.props.name'), 'required');
      } elseif (strlen($title) < 2 || strlen($title) > 500) {
        $errors[] = $this->setError($rowNo, trans('master.term.props.name'), 'min_max', ['min' => 2, 'max' => 500]);
      }
    }
