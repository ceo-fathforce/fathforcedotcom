...
class PortfolioImport implements ToCollection, WithHeadingRow
{
  ...
  public function collection(Collection $rows)
  {
    ...
    $logFile = $this->getLogFile('portfolio');

    $errors = $this->validate($rows);

    $this->checkForErrors('portfolio', $errors);
  ...
  private function import(Collection $rows)
  {
    ...
    foreach ($rows as $row) {
      $portfolio = Portfolio::forceCreate([
  ...
  private function validate(Collection $rows)
  {
    ...
    foreach ($rows as $index => $row) {
      ...
      if (!$title) {
        $errors[] = $this->setError($rowNo, trans('master.portfolio.props.title'), 'required');
      } elseif (strlen($title) < 2 || strlen($title) > 500) {
        $errors[] = $this->setError($rowNo, trans('master.portfolio.props.title'), 'min_max', ['min' => 2, 'max' => 500]);
      }
    }
