...
class LandingtextImport implements ToCollection, WithHeadingRow
{
  use ItemImport;
  ...
  public function collection(Collection $rows)
  {
    ...
    $logFile = $this->getLogFile('landingtext');

    $errors = $this->validate($rows);

    $this->checkForErrors('landingtext', $errors);
  ...
  private function import(Collection $rows)
  {
    ...
    foreach ($rows as $row) {
      $landingtext = Landingtext::forceCreate([
  ...
  private function validate(Collection $rows)
  {
    ...
    foreach ($rows as $index => $row) {
      ...
      if (!$title) {
        $errors[] = $this->setError($rowNo, trans('customize.landingtext.props.name'), 'required');
      } elseif (strlen($title) < 2 || strlen($title) > 500) {
        $errors[] = $this->setError($rowNo, trans('customize.landingtext.props.name'), 'min_max', ['min' => 2, 'max' => 500]);
      }
    }
