...
class GeneralimageImport implements ToCollection, WithHeadingRow
{
  use ItemImport;
  ...
  public function collection(Collection $rows)
  {
    ...
    $logFile = $this->getLogFile('generalimage');

    $errors = $this->validate($rows);

    $this->checkForErrors('generalimage', $errors);
  ...
  private function import(Collection $rows)
  {
    ...
    foreach ($rows as $row) {
      $generalimage = Generalimage::forceCreate([
  ...
  private function validate(Collection $rows)
  {
    ...
    foreach ($rows as $index => $row) {
      ...
      if (!$title) {
        $errors[] = $this->setError($rowNo, trans('customize.generalimage.props.name'), 'required');
      } elseif (strlen($title) < 2 || strlen($title) > 500) {
        $errors[] = $this->setError($rowNo, trans('customize.generalimage.props.name'), 'min_max', ['min' => 2, 'max' => 500]);
      }
    }
